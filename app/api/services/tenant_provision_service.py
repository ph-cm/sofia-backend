...
class TenantProvisionService:
    ...
    def provision_chatwoot(db: Session, user_id: int, account_name: str | None, inbox_name: str | None) -> TenantIntegration:
        ...
        # Retorno antecipado se já houver IDs (estado original)
        ...
        if not account_id:
            raise HTTPException(status_code=502, detail=f"Erro Chatwoot Account: {acc}")
        ...
        if not inbox_id:
            raise HTTPException(status_code=502, detail=f"Erro Chatwoot Inbox: {inbox}")

        # Salva apenas o básico do Chatwoot
